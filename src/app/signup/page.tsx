...
import { ArrowLeft, Home, Wrench, Eye, EyeOff } from "lucide-react";
...
function SignupContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (signUpError) {
        if (signUpError.message.includes("Invalid API key") || signUpError.message.includes("fetch failed")) {
    ...
    <div className="w-full max-w-md">
      ...
      {step === 1 && (
        <div className="space-y-4">
          ...
          <p className="text-center text-sm text-[#565E74] mt-4">
            ...
            <Link href="/login" className="text-[#002B95] font-semibold hover:text-[#001B63]">Sign in</Link>
          </p>
        </div>
      )}
      ...
      {step === 2 && (
        <div className="bg-white rounded-[20px] border border-[#E2E6F0] p-7 shadow-sm">
          ...
          {error && (
            <div className="bg-red-50 border border-red-100 text-red-700 text-sm p-3 rounded-[10px] mb-4">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <p className="text-xs text-[#9EA6BE]">
              ...
              <Link href="/terms" className="text-[#002B95] hover:underline">Terms of Service</Link> and{" "}
              <Link href="/privacy-policy" className="text-[#002B95] hover:underline">Privacy Policy</Link>.
            </p>
          ...
          <button
            onClick={() => router.push(role === "provider" ? "/provider" : "/customer")}
...
export default function SignupPage() {
  return (
    <div className="min-h-screen bg-[#F8F9FC] flex items-center justify-center p-4">
      <Suspense fallback={<div className="text-center text-[#565E74] text-sm">Loading...</div>}>
        <SignupContent />
      </Suspense>
    </div>
  );
